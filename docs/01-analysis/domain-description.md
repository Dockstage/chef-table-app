...
| BRULE-005 | Аллергии содержат не более 300 символов и не должны попадать в технические логи. |
...
| BRULE-009 | Повтор запроса с тем же `Idempotency-Key` и payload возвращает прежний результат; иной payload с тем же ключом отклоняется. |
...
| BRULE-013 | Отзыв разрешён только для `attended`, один раз, с целой оценкой 1–5 и комментарием до 500 символов. |
