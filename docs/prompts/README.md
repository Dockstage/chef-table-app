...
Проверено 26.09.2026: последовательность `000`–`026` непрерывна; каждый PROMPT `001`–`026` связан минимум с одной TASK/BUG-карточкой; каждая TASK `001`–`026` и каждый BUG имеют промпт; ссылки существуют. При новом запросе номер не переиспользуется, а карточка и индекс обновляются в том же изменении.
