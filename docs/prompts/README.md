...
Проверено 26.09.2026: последовательность `000`–`031` непрерывна; каждый PROMPT `001`–`031` связан минимум с одной TASK/BUG-карточкой; каждая TASK `001`–`031` и каждый BUG имеют промпт; ссылки существуют. При новом запросе номер не переиспользуется, а карточка и индекс обновляются в том же изменении.
