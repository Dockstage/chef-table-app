...
Expo-клиент расположен в `client/`. В `backend/` созданы слоистый FastAPI-каркас, SQLAlchemy-модели, Alembic-миграция, seed и служебный health endpoint; PostgreSQL и сервис воспроизводимо запускаются корневым Docker Compose. Доменные endpoint ещё не реализованы. `MockStudioApi` сохраняется только как demo/test fallback.
