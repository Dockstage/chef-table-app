# Чек-лист сдачи

## Уже готово

- [x] Выбран один бриф — «Кулинарная студия».
- [x] Описаны требования MVP, user stories и сценарии.
- [x] Подготовлены архитектурный план, схема данных и OpenAPI.
- [x] Реализованы минимум 3 функции — фактически 8 пользовательских возможностей.
- [x] Составлены 25 тест-кейсов; автоматический регресс содержит 24 теста.
- [x] Найдены и исправлены 3 бага.
- [x] Для задач, багов и промптов созданы `.md`-файлы.
- [x] Выполнены отдельные Git-коммиты.
- [x] TypeScript, тесты и web-сборка проходят.
- [x] Автор всех коммитов — `Gagik Asatryan <100160634+Dockstage@users.noreply.github.com>`.
- [x] Репозиторий опубликован на GitHub.
- [x] Добавлены снимки ручной проверки новых сценариев.
- [x] Реализованы HTTP-адаптер и клиентская push-интеграция.
- [x] `.env` и локальные артефакты исключены из Git.
- [x] TypeScript-модель, ER-схема и OpenAPI согласованы.
- [x] Полный исходный бриф сохранён в `docs/source/`.

## Перед отправкой формы

1. Открыть [репозиторий](https://github.com/Dockstage/chef-table-app) в режиме инкогнито и убедиться, что он доступен проверяющим.
2. В форму задания вставить ссылку на репозиторий.
3. В инструментах указать: Codex, Expo, React Native, TypeScript, Vitest, Expo Notifications.
4. При необходимости заменить демонстрационные имя и телефон в `client/App.tsx` — на оценивание функций это не влияет.

Нативная доставка push на физическом Android/iOS не проверяется по решению владельца проекта; клиентские ветви foreground, открытия уведомления и холодного запуска покрыты автоматическими тестами.

Перед отправкой ещё раз выполнить:

```bash
cd client
npm ci
npm run typecheck
npm test
npm run export:web
cd ..
git status
```

`git status` должен показать чистую рабочую ветку.
